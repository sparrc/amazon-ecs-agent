...
const (
	...
	logPathHostRoot                             = "/var/log/ecs/daemons"
)
...
func (dm *daemonManager) CreateDaemonTask() (*apitask.Task, error) {
	...
	// (CMD for example)
	//daemonUIDString := strconv.Itoa(daemonUID)
	rawConfig = fmt.Sprintf("{\"Healthcheck\":%s, \"User\":\"0\"}", string(rawHealthConfig))
	logger.Info(fmt.Sprintf("Raw Config: %s", rawConfig))
...
func (dm *daemonManager) initDaemonDirectoryMounts(imageName string) error {
	...
	daemonLogPath := filepath.Join(logPathHost, "log")
	logger.Info(fmt.Sprintf("daemonLogPath is: %s", daemonLogPath))
	if err := mkdirAllAndChown(daemonLogPath, daemonMountPermission, daemonUID, os.Getegid()); err != nil {
		return err
	}
	return nil
}
